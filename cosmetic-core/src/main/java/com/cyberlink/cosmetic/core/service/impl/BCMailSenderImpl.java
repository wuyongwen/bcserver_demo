...
public class BCMailSenderImpl extends AbstractService implements BCMailSender {
	private String from;
	...
	@Transactional(propagation = Propagation.REQUIRES_NEW, isolation = Isolation.READ_UNCOMMITTED)
	public void sendMimeMessageWithBcc(final String subject, final String content,
			final String[] bccEmail, final String... email) {
		final Map<String, File> attachements = Collections.emptyMap();
		sendMimeMessageWithBcc(subject, content, attachements, bccEmail, email);

	}

	@Transactional(propagation = Propagation.REQUIRES_NEW, isolation = Isolation.READ_UNCOMMITTED)
	public void sendMimeMessageWithBcc(final String subject, final String content,
			final Map<String, File> attachments, final String[] bccEmail, final String... email) {
		final MimeMessagePreparator preparator = new MimeMessagePreparator() {
			public void prepare(MimeMessage mimeMessage) throws Exception {
				final MimeMessageHelper helper = new MimeMessageHelper(
						mimeMessage, !attachments.isEmpty(), "UTF-8");
