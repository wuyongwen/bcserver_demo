package com.cyberlink.cosmetic.utils.ahoCorasick;
...
public class AhoCorasickDoubleArrayTrie<V> implements Serializable
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3993343111093164260L;
	/**
     ...
     */
    public Boolean hasKeyword(String text, IHit<V> processor)
    {
        int position = 1;
        int currentState = 0;
        for (int i = 0; i < text.length(); ++i)
        {
            ...
            if (hitArray != null)
            {
                ...
                {
                     if(processor.isHit(position - l[hit], position, v[hit]))
                    	 return Boolean.TRUE;
                }
            }
            ++position;
