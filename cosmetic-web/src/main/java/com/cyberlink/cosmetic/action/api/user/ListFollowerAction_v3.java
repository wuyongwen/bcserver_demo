...
@UrlBinding("/api/v3.0/user/list-follower.action")
public class ListFollowerAction_v3 extends AbstractAction{
    ...
    public Resolution route() {
        ...
        if (totalFollowrCount == null) {
        	subscriberIds = subscribeDao.findBySubscribeeOrderByName(userId, null, new BlockLimit(offset.intValue(), limit.intValue()), true);
        } else {
        	subscriberIds = subscribeDao.findBySubscribeeOrderByName(userId, null, new BlockLimit(offset.intValue(), limit.intValue()), false);
        ...
        if (curUserId != null) {
        	subscribeSet = subscribeDao.findIdBySubscriberAndSubscribees(
        			curUserId, SubscribeType.User, subscriberIds.getResults().toArray(new Long[subscriberIds.getResults().size()]));
        }
