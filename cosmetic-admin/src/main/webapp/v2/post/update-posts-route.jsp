...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(".add_image_btn2").each(function(index, element) {
            CKEDITOR.replace($(element).find(".add_url")[0]);
        });
    });
        ...
        <div id="broadcastMessageApp" ng-app="broadcastMessageApp" ng-controller="broadcastMessageCtrl" ngcloak="" class="ng-scope">
            <div id="createPostView">
                ...
                <div class="group-select">
                    <label>Title:<span class="info"></span></label>
                    <br>
                    <s:text name="posts[0].title" id="title" class="add_title" maxlength="45"/>
                </div>
                <div class="group-select">
                ...
                <div id="postContent">
                    <div class="add-paragraph">
                        ...
                        <div id="photoSet">
                            <c:forEach items="${actionBean.posts}" var="post" varStatus="loop">
                                <div class="add_image_btn2" id="${loop.index}">
                                    <c:choose>
                                        <c:when test="${loop.index eq 0}">
                                            <div class="delete_paragraph" hidden="true"><a href="javascript: void(0);" ><i class="fa fa-times" style="color:#FFFFFF;"></i></a></div>
                                        </c:when>
                                        <c:otherwise>
                                            <div class="delete_paragraph"><a href="javascript: void(0);" ><i class="fa fa-times" style="color:#FFFFFF;"></i></a></div>
                                        </c:otherwise>
                                    </c:choose>
                                    <h2 style="margin-left:15px;">Paragraph ${loop.index+1}</h2>
                                    <ul>
                                        <li>
                                            <div class="postImgContainer">
                                                <img class="postImg" src="${post.attachments[0].attachmentFile.fileItems[0].originalUrl}">
                                                <input class="postImgInput" name="file" type="file" accept="image/*">
                                                <div class="fileId" style="display:none;">${post.attachments[0].attachmentFile.fileItems[0].file.id}</div>
                                            </div>
                                            <c:choose>
                                                <c:when test="${empty post.redirectUrl}">
                                                    <div class="addRedirectUrlBtn"><a href="javascript: void(0);">Add URL</a></div>
                                                </c:when>
                                                <c:otherwise>
                                                    <div class="addRedirectUrlBtn"><a href="javascript: void(0);">Edit URL</a></div>
                                                </c:otherwise>
                                            </c:choose>
                                            
                                            <input class="redirectUrl" name="input" type="text" maxlength="1024" hidden="true" value="${post.redirectUrl}" isWidget="${post.isWidget}">
                                        </li>
                                    </ul>
                    ...
                    <c:if test="${actionBean.posts[0].postStatus eq 'Hidden'}">
	                    <div style="font-size: 15px">
	          				<p><span>Schedule posting</span></p>
	        				<label>
	           					<input id="scheduleCheck" type="checkbox" value="checkbox" checked disabled style="visibility:hidden;"><span> Publish on specified date & time</span></input>
	        				</label><br>
							<input type="text" id="scheduleDateTime" readonly='true' value="${actionBean.posts[0].createdTime}" zone="${actionBean.currentUser.region}" />
	      					</div>
	      				</div>
      				</c:if>
...
<!--                             <div class="previewurl_timeinfo">
            ...
            <div class="post_create_btn">
                <div class="post_create_publishbtn" id="updateBtn"><a href="javascript: void(0);">Update</a></div>
