...
try {
	...
	if (Constants.getWebsiteDomain().equalsIgnoreCase("bc-api-eu.cyberlink.com")) {
		userIds.add("46001");
		userIds.add("48001");
		//userIds.add("50001");
	} else if (Constants.getWebsiteDomain().equalsIgnoreCase("bc-api-jp.cyberlink.com")) {
		userIds.add("40001");
		userIds.add("42001");
		userIds.add("44001");
	} else {
	...
	/*for (String userId : userIds) {
		...
		returnJson = urlContentReader.post("http://" + Constants.getWebsiteDomain() + "/api/post/list-post-by-user.action", params);
	...
	returnJson = urlContentReader.post("http://" + Constants.getWebsiteDomain() + "/api/post/list-sub-post.action", params);
	contentMap = objectMapper.readValue(returnJson, Map.class);
	mapArray = (List<Map<String, Object>>) contentMap.get("results");
	...
	returnJson = urlContentReader.post("http://" + Constants.getWebsiteDomain() + "/api/feed/list-my-feed.action", params);
	contentMap = objectMapper.readValue(returnJson, Map.class);
	mapArray = (List<Map<String, Object>>) contentMap.get("results");
	if (!mapArray.get(0).containsKey("postId")) {	
		out.print("fail");
	...
	for (String circleId : circleIds.keySet()) {
		...
		returnJson = urlContentReader.post("http://" + Constants.getWebsiteDomain() + "/api/post/list-post-by-circle.action", params);
		contentMap = objectMapper.readValue(returnJson, Map.class);
		mapArray = (List<Map<String, Object>>) contentMap.get("results");
		if (!mapArray.get(0).containsKey("postId")) {	
			out.print("fail");
	...
	returnJson = urlContentReader.post("http://" + Constants.getWebsiteDomain() + "/api/post/list-comment.action", params);
	contentMap = objectMapper.readValue(returnJson, Map.class);
	mapArray = (List<Map<String, Object>>) contentMap.get("results");
